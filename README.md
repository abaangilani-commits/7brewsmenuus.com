...
Structured data: `node tools/build-schema.js` writes the Organization + WebSite entity, category Menu/MenuSection/MenuItem graphs and missing FAQPage blocks into the hand-written pages (between SCHEMA:SITE markers). Generated pages get the same site entity via tools/site-entity.js. `node tools/sync-card-nutrition.js` copies published calories and caffeine from the nutrition guide into menu-data.js and the homepage cards.
